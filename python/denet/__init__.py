...
from denet._denet import ProcessMonitor, generate_summary_from_file, generate_summary_from_metrics_json
                        ...
                        if os.name == 'posix':  # Unix-based systems
                            ...
                            if metrics_json is not None:
                                ...
                                if output_file_path:
                                    ...
                                    is_new_file = not os.path.exists(output_file_path) or os.path.getsize(output_file_path) == 0
                                    
                                    with open(output_file_path, 'a') as f:
                                        if is_new_file:
                                            # Add metadata as first line
                                            metadata = {"pid": pid, "cmd": ["python"], "executable": sys.executable, "t0_ms": int(time.time() * 1000)}
                ...
                while self.monitoring:
                    ...
                    if os.name == 'posix':  # Unix-based systems
                        ...
                        if metrics_json is not None:
                            ...
                            if self.output_file:
                                # Check if file is empty/new and write metadata first
                                is_new_file = not os.path.exists(self.output_file) or os.path.getsize(self.output_file) == 0
                                
                                with open(self.output_file, 'a') as f:
                                    if is_new_file:
                                        # Add metadata as first line
                                        metadata = {"pid": self.pid, "cmd": ["python"], "executable": sys.executable, "t0_ms": int(time.time() * 1000)}
                                        f.write(json.dumps(metadata) + '\n')
                                    f.write(metrics_json + '\n')
