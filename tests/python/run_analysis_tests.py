...
def aggregate_metrics(metrics, window_size=10, method="mean"):
    ...
    for i in range(0, len(metrics), window_size):
        ...
        numeric_fields = [
            "cpu_usage", "mem_rss_kb", "mem_vms_kb",
            "disk_read_bytes", "disk_write_bytes",
            "net_rx_bytes", "net_tx_bytes",
            "thread_count", "uptime_secs"
        ]
...
class TestAnalysisUtilities(unittest.TestCase):
    ...
    def test_resource_utilization(self):
        ...
        self.assertAlmostEqual(stats["avg_cpu"], 9.0, delta=0.1, 
                             msg=f"Expected avg CPU of 9.0, got {stats['avg_cpu']}")
        
        # Check memory statistics
        self.assertIn("avg_mem_mb", stats)
        self.assertIn("max_mem_mb", stats)
        # Just check the value is close (between 7.5 MB and 8.5 MB)
        self.assertTrue(7.5 <= stats["max_mem_mb"] <= 8.5, 
    ...
    def test_process_tree_analysis(self):
        ...
        self.assertAlmostEqual(tree_analysis["main_process"]["avg_cpu"], 7.5, delta=0.1,
        ...
        self.assertAlmostEqual(tree_analysis["child_processes"][1001]["avg_cpu"], 3.5, delta=0.1,
    ...
    def test_save_and_load_metrics(self):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.jsonl') as jsonl_file, \
